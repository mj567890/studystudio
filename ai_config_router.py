...
KNOWN_CAPABILITIES = [
    ...
    {"key": "reranker",              "group": "vector", "kind": "chat",      "label": "检索重排",            "required": False},
    # ── 多模态（未来）──
...
class TestProviderRequest(BaseModel):
    ...
    model_name: str
    capability_kind: str = Field(..., pattern="^(chat|embedding)$")
    ...
    try:
        if req.capability_kind == "chat":
            ...
            result = {
                ...
                "dimension_mismatch": dim != cur_dim,
...
async def migrate_embedding_dimension(
    ...
    logger.warning("Embedding dimension migration completed",
                   old_dim=cur_dim, new_dim=new_dim)
    return {"code": 200, "data": {
        "message": f"维度已从 {cur_dim} 迁移到 {new_dim}。"
