...
class EventBus:
    ...
    async def reset(self) -> None:
        """强制重置连接（Celery fork 后 event loop 变更时使用）。"""
        if self._connection:
            try:
                await self._connection.close()
            except Exception:
    ...
    async def subscribe(
        ...
        *,
        ...
        # 声明队列（仅 durable，不带额外参数）
        # 不加 x-dead-letter-exchange / x-message-ttl：
        # blueprint.synthesis.queue 等队列同时被 Celery worker 声明（无参数），
        # 带参数声明会导致 PRECONDITION_FAILED，API 无法启动。
