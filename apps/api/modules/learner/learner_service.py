...
class GapScanService:
    ...
    async def scan(self, user_id: str, topic_key: str) -> dict:
        ...
        result = await self.db.execute(
            ...
            """),
            {"user_id": user_id}  # scan_uid_fix_v1
        )
        rows = result.fetchall()
...
class RepairPathService:
    ...
    async def compute(
        ...
        if space_id:
            bp_row = await self.db.execute(
                text("""
                    ...
                    WHERE sb.topic_key = :tk AND sb.space_id = CAST(:sid AS uuid) AND sb.status = 'published'
                    LIMIT 1
                """),
        ...
        bp = bp_row.fetchone()
        if bp:
            return await self._compute_chapter_path(user_id, topic_key, str(bp[0]), space_id=space_id)
    ...
    async def _compute_chapter_path(self, user_id: str, topic_key: str, blueprint_id: str, space_id: str | None = None) -> dict:
        """基于已发布蓝图，返回 chapter 级学习路径。"""
        # 获取所有章节（含 content_text 用于估算学习时间）
        chapters_result = await self.db.execute(
            text("""
                SELECT sc.chapter_id::text, sc.title, sc.objective,
                       ...
                       ss.stage_order, sc.content_text
        ...
        if space_id:
            _prog_sql = """
                ...
                WHERE user_id = CAST(:uid AS uuid)
                  ...
                  AND completed = true
            """
            _prog_params = {"uid": user_id, "tk": topic_key, "sid": space_id}
        else:
            _prog_sql = """
                SELECT chapter_id::text FROM chapter_progress
                WHERE user_id = CAST(:uid AS uuid)
                  ...
                  AND completed = true
            """
            _prog_params = {"uid": user_id, "tk": topic_key}
        progress_result = await self.db.execute(text(_prog_sql), _prog_params)
        ...
        for ch in chapters:
            ...
            # 6.3: 基于内容长度估算学习时间（100 字 ≈ 1 分钟，最少 5 分钟）
            estimated_minutes = 20  # 默认
            try:
                ct = ch.get("content_text")
                if ct:
                    if isinstance(ct, str):
                        data = json.loads(ct)
                    else:
                        data = ct
                    full_text = data.get("full_content", "") if isinstance(data, dict) else ""
                    if full_text:
                        estimated_minutes = max(5, min(60, len(full_text) // 100))
            except Exception:
                pass
