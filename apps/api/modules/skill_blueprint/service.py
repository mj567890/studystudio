...
class BlueprintService:
    ...
    async def get_blueprint(self, topic_key: str, space_id: str | None = None) -> BlueprintOut | None:
        bp = await self.repo.get_published_by_topic(topic_key, space_id=space_id)
        if not bp:
            return None
        ...
        return BlueprintOut(blueprint_id=bp["blueprint_id"], topic_key=bp["topic_key"],
                            ...
                            space_id=bp.get("space_id"),
                            stages=stages, created_at=bp["created_at"],
                            updated_at=bp["updated_at"])

    async def get_status(self, topic_key: str, space_id: str | None = None) -> BlueprintStatusOut:
        bp = await self.repo.get_by_topic(topic_key, space_id=space_id)
        if not bp:
    ...
    async def publish(self, topic_key: str, space_id: str | None = None) -> BlueprintStatusOut:
        bp = await self.repo.get_by_topic(topic_key, space_id=space_id)
        if not bp:
