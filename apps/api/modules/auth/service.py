...
def _normalize_uuid_str(value: str | UUID) -> str:
...
class AuthService:
    ...
    async def register(self, email: str, password: str, nickname: str) -> dict:
        ...
        await self.db.execute(
            text("""
                ...
                VALUES (CAST(:user_id AS uuid), :email, :password_hash, :nickname)
            """),
            {
                "user_id": user_id,
                "email": email,
        ...
        await self.db.execute(
            text("""
                ...
                SELECT CAST(:user_id AS uuid), role_id
                FROM roles
        ...
        await self.db.execute(
            text("INSERT INTO learner_profiles (user_id) VALUES (CAST(:user_id AS uuid))"),
            {"user_id": user_id}
        )
    ...
    async def login(self, email: str, password: str) -> dict:
        ...
        logger.info("User logged in", user_id=str(row.user_id))
        return {"access_token": token, "token_type": "bearer", "roles": roles}

    async def get_current_user(self, user_id: str | UUID) -> dict:
        uid = _normalize_uuid_str(user_id)
        result = await self.db.execute(
            text("""
                SELECT user_id, email, nickname, avatar_url, status, created_at
                FROM users
        ...
        roles = await self._get_user_roles(uid)
        return {
    ...
    async def _get_user_roles(self, user_id: str | UUID) -> list[str]:
        uid = _normalize_uuid_str(user_id)
        result = await self.db.execute(
            text("""
                SELECT r.role_name
    ...
    async def check_permission(self, user_id: str | UUID, permission_code: str) -> bool:
        uid = _normalize_uuid_str(user_id)
        result = await self.db.execute(
            text("""
                SELECT 1
            ...
            """),
            {"user_id": uid, "code": permission_code}
        )
