...
async def _synthesize_blueprint_merge_async(
    topic_key: str, space_id: str, existing_blueprint_id: str, prior_updated_at: str = None, teacher_instruction: str | None = None
) -> None:
    """
    ...
    async with SF() as session:
        async with session.begin():
            ...
            inserted = await _insert_new_chapters(
                repo, llm, diff["new_topic"], existing_blueprint_id, session, instruction_block
            )
...
def synthesize_blueprint(self, topic_key: str, space_id, teacher_instruction: str | None = None):
    ...
    if space_id:
        try:
            ...
            if existing_bp_id:
                ...
                # 否则 _check_blueprint_lock 会更新 updated_at=NOW()
                # 导致 _diff_new_entities 找不到任何新实体
                ...
                try:
                    asyncio.run(_synthesize_blueprint_merge_async(
                        topic_key, str(space_id), existing_bp_id, prior_updated_at, teacher_instruction
                    ))
                except Exception as exc:
    ...
    try:
        asyncio.run(_synthesize_blueprint_v2_async(topic_key, space_id, teacher_instruction))
    except Exception as exc:
...
async def _synthesize_blueprint_v2_async(topic_key: str, space_id, teacher_instruction: str | None = None) -> None:
    import numpy as np
