...
async def _insert_new_chapters(
    repo, llm, new_topic_entities: list[dict], blueprint_id: str, session
) -> int:
    """
    ...
    async with SF() as session:
        async with session.begin():
            ...
            inserted = await _insert_new_chapters(
                repo, llm, diff["new_topic"], existing_blueprint_id, session
            )
...
async def _synthesize_blueprint_v2_async(topic_key: str, space_id) -> None:
    ...
    stage_plan = await _plan_stages_with_llm(llm, chapters_for_planning, len(cluster_chapters))
