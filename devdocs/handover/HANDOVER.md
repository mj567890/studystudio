...
**生成日期：** 2026-04-29
**上次更新主题：** 教师引导式课程迭代三层方案 + 服务器部署 + 类型不匹配修复
...
## 三-K、本次会话完成工作（2026-04-29 服务器 Bug 修复 + 生产部署）

### Bug：chapter_progress JOIN 类型不匹配 500 错误

**现象：** `GET /api/learners/me/dashboard` 返回 500
...
**根因：** 迁移 026 将 `chapter_progress.chapter_id` 从 VARCHAR 改为 UUID，但 `eight_dim_endpoints.py` 中 6 处 JOIN 仍保留 `sc.chapter_id::text = cp.chapter_id` 写法，产生 `text = uuid` 类型不匹配。

**修复：** `eight_dim_endpoints.py` 中 6 处移除多余的 `::text` 转换：

| 行号 | 旧写法 | 新写法 |
|------|--------|--------|
| 293 | `cp.chapter_id=sc.chapter_id::text` | `cp.chapter_id=sc.chapter_id` |
| 420 | `cp.chapter_id = sc_tgt.chapter_id::text` | `cp.chapter_id = sc_tgt.chapter_id` |
| 820 | `cp.chapter_id = sc.chapter_id::text` | `cp.chapter_id = sc.chapter_id` |
| 1517 | `sc.chapter_id::text = cp.chapter_id` | `sc.chapter_id = cp.chapter_id` |
| 1570 | `sc.chapter_id::text = cp.chapter_id` | `sc.chapter_id = cp.chapter_id` |
| 1594 | `cp.chapter_id = sc.chapter_id::text` | `cp.chapter_id = sc.chapter_id` |

**提交：** `a380b77`

### 生产部署（2026-04-29）

服务器代码同步 → DDL 迁移 027 执行（5 列全部成功）→ API + Celery 容器重启 → Web 容器重建（npm build + nginx）→ 验证 10 容器全部正常运行。

部署后 `dashboard` 500 错误由用户反馈发现并修复。

### 变更文件

| 文件 | 变更 | 说明 |
|------|------|------|
| `apps/api/modules/learner/eight_dim_endpoints.py` | 修复 | 6 处 `::text` 转型移除 |
...
## 三-J、本次会话完成工作（2026-04-29 教师引导式课程迭代三层方案）

### 背景

当前 AI 课程生成是"黑盒"模式：上传资料 → AI 自动生成全套内容。教师无法注入教学意图，事后手动修改导致章节与习题/讨论脱节。

核心问题：**AI 不是教师，不懂学情、专业侧重、授课思路。**

设计讨论产出三层递进方案（详见 `删除功能设计讨论_20260428.md` 末尾）：
- Layer 1：生成前约束 — 教师提前注入教学要求，AI 从源头按约束生成
- Layer 2：对话式章节精调 — 教师用自然语言指令让 AI 定向重写单章
- Layer 3：附属内容联动 — 章节变更后测验 + 讨论自动刷新

### Layer 1：生成前约束

**后端：**
- `GenerateBlueprintRequest` schema 新增 `teacher_instruction` 字段
- Router 传递 `teacher_instruction` 到 `synthesize_blueprint` Celery 任务（第 3 个 arg）
- `BlueprintRepository` 新增 `set_teacher_instruction()`，V2 函数中蓝图创建后持久化
- 4 个 prompt 模板（`CHAPTER_CONTENT_PROMPT`/`CLUSTER_CHAPTER_PROMPT`/`COURSE_TITLE_PROMPT`/`STAGE_PLANNING_PROMPT`）均加 `{teacher_instruction}` 占位符
- `_synthesize_blueprint_v2_async` 计算 `instruction_block = TEACHER_INSTRUCTION_PREFIX.format(instruction=...)`，注入所有 `.format()` 调用
- Merge 模式（`_synthesize_blueprint_merge_async` + `_insert_new_chapters`）同步支持
- 新增常量 `TEACHER_INSTRUCTION_PREFIX`：教师要求优先级最高

**前端：**
- `RepairPathView.vue` 新增教学指导 textarea（在"生成路径"按钮上方）
- `blueprintApi.generate()` 签名扩展，支持 `teacherInstruction` 参数

**效果：** 教师可在生成前输入"侧重实操，减少理论推导，适配中职基础"等约束，AI 从源头按规则生成全套内容。

### Layer 2：对话式章节精调

**后端：**
- 新增 `CHAPTER_REFINEMENT_PROMPT`：教师指令 + 当前章节摘要 + 全局教学约束 → LLM 重写完整 JSON
- 新增 `POST /admin/courses/chapters/{chapter_id}/refine` 端点（同步执行，~150s timeout）
- 读取蓝图级 `teacher_instruction` 作为全局约束注入精调 prompt
- 章节更新后递增 `refinement_version`，记录 `refined_at` 时间戳
- 内嵌 Layer 3 触发逻辑（测验失效 + 派发异步任务）

**前端：**
- `KnowledgeView.vue` 章节操作列新增"精调"按钮（黄色，预览/重生成之间）
- `ElMessageBox.prompt` 弹窗输入自然语言指令
- `adminApi.refineChapter()` API 封装

**效果：** 管理员点击"精调"→ 输入"增加航空维修安全规范，加入真实事故案例"→ AI 定向重写本章内容。

### Layer 3：附属内容联动

**后端：**
- refine 端点中自动执行 `UPDATE chapter_quizzes SET generated_at = NULL`（失效旧缓存）
- 新增 `regenerate_chapter_quiz` Celery 任务：读取章节关联知识点 → 复用 `QUIZ_GENERATION_PROMPT` → 重新生成测验 → 写入 `chapter_quizzes`
- 新增 `generate_discussion_seeds` Celery 任务：读取章节内容摘要 → `DISCUSSION_SEED_PROMPT` → 写入 `course_posts`（`post_type='discussion'`）
- 教师可通过 `auto_regenerate_quiz`/`auto_regenerate_discussion` 参数控制是否触发联动
...
- **"教师教学意图前置注入"**：生成前约束，从源头对齐授课目标
- **"自然语言指令迭代课程"**：抛弃手动逐字编辑，对话式指挥 AI 定向修改
- **"附属内容智能联动"**：章节变更 → 测验 + 讨论自动刷新，人工可控
- **架构轻量**：按章节独立闭环，跨章隔离，不产生全局连锁耦合

### 变更文件一览

| 文件 | 变更 | 说明 |
|------|------|------|
| `migrations/027_teacher_refinement.sql` | **新增** | 5 列 DDL |
| `apps/api/modules/skill_blueprint/schema.py` | 修改 | `GenerateBlueprintRequest` 加 `teacher_instruction` |
| `apps/api/modules/skill_blueprint/router.py` | 修改 | 传递 `teacher_instruction` 到 Celery |
| `apps/api/modules/skill_blueprint/repository.py` | 新增 | `set_teacher_instruction()` |
| `apps/api/tasks/blueprint_tasks.py` | 修改 | 4 prompt +占位符，4 新常量/任务（+242/-60 行） |
| `apps/api/modules/admin/router.py` | 新增 | `refine_chapter` 端点（+146 行） |
| `apps/web/src/views/learner/RepairPathView.vue` | 修改 | 教学指导 textarea + 状态 |
| `apps/web/src/views/admin/KnowledgeView.vue` | 修改 | 精调按钮 + refineChapter 函数 |
| `apps/web/src/api/index.ts` | 修改 | `blueprintApi.generate()` + `adminApi.refineChapter()` |
| `apps/web/dist/` | 重构建 | 含全部前端变更 |

### 验证结果

- 5 个后端 Python 文件语法检查全部通过
- 前端 `npm run build` 构建成功（56KB 主入口 + chunk 拆分正常）
- 向后兼容：`teacher_instruction` 为空时所有 prompt 与原有字节完全一致
