...
class DiagnosisWriteService:
    ...
    async def update_with_lock(
        ...
        """
        result = await self.db.execute(
            text("SELECT version, mastery_summary FROM learner_profiles WHERE user_id = :uid"),
            {"user_id": user_id}
        )
        row = result.fetchone()
        if not row:
    ...
    async def _apply_diagnosis(
        ...
        await self.db.execute(
            text("""
                ...
                SET mastery_summary = mastery_summary || :update::jsonb,
                    version = version + 1,
                    updated_at = NOW()
...
class TeachingChatService:
    ...
    async def chat_and_prepare(
        ...
        profile_result = await self.db.execute(
            text("SELECT mastery_summary, version FROM learner_profiles WHERE user_id = :uid"),
            {"user_id": user_id}
        )
        ...
        await self.db.execute(
            text("""
                ...
                VALUES (:tid, :cid, 'assistant', :content, :gap_type, :error_pattern, :cited::jsonb)
            """),
            {
                "tid":           turn_id,
                "cid":           conversation_id,
