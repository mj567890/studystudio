...
- selected_cases → 分配到案例最适合出现的章节（1 个案例可分配 1-2 章）
...
def validate_calibration_questions(questions: list, entities: list) -> list[str]:
    ...
    for q in questions:
        ...
        # 选项不能全是材料原句（检查与 entity.short_definition 的相似度）
        original_similarity = max([
            keyword_overlap(o['label'], get_entity_definition(o.get('entity_id')))
            for o in q.get('options', []) if o.get('entity_id')
        ] or [0])
        if original_similarity > 0.9:
            issues.append(f"{q['id']}: 选项过于接近材料原文，缺乏'需要真实经验才能选'的迷惑性")
    # 题2 和 题5 必须有"让我补充"
...
2. **上一版作为反例**：重新规划时将上一版 Course Map 和教师选的原因注入 prompt：
   `"上一版方案是：[...]，教师认为 [原因]。请尝试明显不同的章节切分方式，避开上一版的问题。"`

3. **上限 3 次**：超过 3 次后强制教师在已有版本中选择一个确认，避免无限循环。
...
**重要**：以上数据已经由 Course Map 精确路由到本章。不要将不属于本章的隐性经验强行编入。例如，理论课只获得与该理论概念相关的误区，不应出现操作层面的红线警告。
